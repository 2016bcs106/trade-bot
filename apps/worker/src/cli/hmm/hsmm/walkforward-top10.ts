...
const SYMBOLS = ["PAYTM", "SWSOLAR", "WELSPUNLIV", "INFOBEAN", "LXCHEM", "DELTACORP", "VEDL", "IRFC", "PARADEEP", "HAPPSTMNDS"];
const N = 3;
const D = 20;
const FOLDS = 5;
...
for (const symbol of SYMBOLS) {
  ...
  for (let fold = 0; fold < FOLDS; fold++) {
    ...
    const upRegime = [...result.emissionParams].sort((a, b) => b.mean - a.mean)[0];
    upMeans.push(upRegime.mean);
    upVars.push(upRegime.variance);
  }

  console.log(`\n${symbol}: n=${observations.length}`);
  console.log(`  duration MAE: all=${avg(foldMaes).toFixed(2)} days, segment-starts=${avg(foldSegMaes).toFixed(2)} days`);
  console.log(`  regime stability across folds: ${(avg(foldStabilities) * 100).toFixed(1)}%`);
  console.log(`  up-regime mean across folds: ${upMeans.map((x) => x.toFixed(4)).join(", ")}`);
  console.log(`  up-regime var across folds:  ${upVars.map((x) => x.toFixed(5)).join(", ")}`);
}
